...
# for a *single* roll-call. The returned matrix uses the same MEP orders for its rows and columns.
#
...
# 
# votes: vector of all MEP votes for the considered roll-call.
...
# (on the rows) for a series of roll-calls (columns). The returned matrix uses the same MEP orders 
...
# MEP who abstained gets a score of zero when compared to himself.
# 
# votes: matrix of all MEP (rows) votes for the considered roll-calls (columns).
...
#############################################################################################
process.agreement.stats <- function(all.votes, rollcall.details, score.file, domains, dates, country, group, plot.formats)
...
# plot.formats: formats used for the plot files.
#############################################################################################
process.agreement <- function(all.votes, rollcall.details, mep.details, score.file, domains, dates, everything, countries, groups, plot.formats)
{	tlog("***************************************************")
	...
	{	tlog("....Process stats for group ",group)
		...
		# process agreement
		process.agreement.stats(group.votes, rollcall.details, score.file, domains, dates, country=NA, group, plot.formats)
	}
	...
	{	tlog("....Process stats for country ",country)
		...
		# process agreement
		process.agreement.stats(country.votes, rollcall.details, score.file, domains, dates, country, group=NA, plot.formats)
	}
	
	# process agreement for all data
	if(everything)
	{	tlog("..Process agreement for all data")
		process.agreement.stats(all.votes, rollcall.details, score.file, domains, dates, country=NA, group=NA, plot.formats)
	}
}
...
#print(indices)
#################################################
#process.agreement(all.votes, rollcall.details, mep.details, score.file="m3")
#################################################
